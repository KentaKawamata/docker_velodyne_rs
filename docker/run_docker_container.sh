...
if [ -z "${DOPE_ID}" ]; then
    ...
    xhost +local:root
    docker run --runtime=nvidia -it --privileged --net host -v ${HOST_DIR}:${CONTAINER_DIR}:rw -v /tmp/.X11-unix:/tmp/.X11-unix:rw -v ~/.Xauthority:/root/.Xauthority:rw --env="DISPLAY=${DISPLAY}" --env="QT_X11_NO_MITSHM=1" --env="PATH=/usr/local/nvidia/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" --env="LD_LIBRARY_PATH=/usr/local/nvidia/lib64" --env="XAUTHORITY=$XAUTH" --name=${CONTAINER_NAME} ${IMAGE_NAME} /bin/bash
